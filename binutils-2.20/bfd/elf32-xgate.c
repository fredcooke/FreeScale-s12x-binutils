...
xgate_elf_bfd_link_hash_table_create (bfd *abfd)
{
  ...
  ret = xgate_elf_hash_table_create (abfd);
  if (ret == (struct xgate_elf_link_hash_table *) NULL)
    return NULL;

  ret->size_one_stub = xgate_elf_size_one_stub;
  ret->build_one_stub = xgate_elf_build_one_stub;
...
xgate_add_stub (const char *stub_name, asection *section,
  ...
  stub_entry = xgate_stub_hash_lookup (htab->stub_hash_table, stub_name,
      TRUE, FALSE);
  if (stub_entry == NULL)
    {
...
int
elf32_xgate_setup_section_lists (bfd *output_bfd, struct bfd_link_info *info)
{
...
bfd_boolean
elf32_xgate_size_stubs (bfd *output_bfd, bfd *stub_bfd,
    struct bfd_link_info *info,
    asection * (*add_stub_section) (const char*, asection*))
{
  ...
  struct xgate_elf_link_hash_table *htab = xgate_elf_hash_table (info);
  ...
  for (input_bfd = info->input_bfds, bfd_indx = 0;
    ...
    for (section = input_bfd->sections;
      ...
      for (; irela < irelaend; irela++)
        ...
        if (r_type != (unsigned int) R_XGATE_16)
          continue;
        ...
        stub_entry = xgate_stub_hash_lookup
        ...
        if (stub_entry == NULL)
          ...
          stub_entry = xgate_add_stub (stub_name, section, htab);
          if (stub_entry == NULL)
            {
...
xgate_elf_export_one_stub (struct bfd_hash_entry *gen_entry, void *in_arg)
{
  ...
  xgate_elf_set_symbol (htab->stub_bfd, info, name,
...
static void
xgate_elf_set_symbol (bfd *abfd, struct bfd_link_info *info,
...
bfd_boolean
elf32_xgate_build_stubs (bfd *abfd, struct bfd_link_info *info)
{
  //printf("\n in build_stubs");
  asection *stub_sec;
  ...
  bfd_hash_traverse (table, xgate_elf_export_one_stub, info);
  ...
  if (param.use_memory_banks)
    {
    xgate_elf_set_symbol (abfd, info, BFD_XGATE_BANK_START_NAME,
        htab->pinfo.bank_physical,
        bfd_abs_section_ptr);
    xgate_elf_set_symbol (abfd, info, BFD_XGATE_BANK_VIRTUAL_NAME,
        htab->pinfo.bank_virtual,
        bfd_abs_section_ptr);
    xgate_elf_set_symbol (abfd, info, BFD_XGATE_BANK_SIZE_NAME,
...
int
xgate_addr_is_banked (struct xgate_page_info *pinfo, bfd_vma addr)
{
  //printf("\n in addr_is_banked");
...
bfd_vma
xgate_phys_addr (struct xgate_page_info *pinfo, bfd_vma addr)
{
  printf("\n in phys addr and address is %lu",(unsigned long)addr);
  if (addr < pinfo->bank_virtual)
    return addr;
...
#undef elf_info_to_howto_rel  /* TODO fix so this is not necessary */
    ...
    if (is_far && ELF32_R_TYPE (rel->r_info) == R_XGATE_16)
      {
      //printf("\n ELF32_R_TYPE is == R_XGATE");

      struct elf32_xgate_stub_hash_entry* stub;
      struct xgate_elf_link_hash_table *htab;

      htab = xgate_elf_hash_table (info);
      stub = xgate_stub_hash_lookup (htab->stub_hash_table,
          name, FALSE, FALSE);
      if (stub)
        {
    ...
    switch (r_type)
    {
    case	R_XGATE_IMM8_LO:
      printf("\n case xgate.c relocation is %lu", relocation);
      /* relocation is already set to physical address at this point */

      break;
    case	R_XGATE_IMM8_HI:
      printf("\n case xgate.c elf32 relocate IMM8_HI relocation is %lu", relocation);
      break;
    case R_XGATE_24:
    ...
    case R_XGATE_16:
      ...
      if (xgate_addr_is_banked (pinfo, relocation + rel->r_addend)
        ...
        sprintf (buf, msg, phys_page, phys_addr,
            ...
            insn_page, xgate_phys_addr (pinfo, insn_addr),
            (long) (insn_addr));
    ...
    if (r_type != R_XGATE_NONE){
      //printf("\n type is not R_XGATE_NONE r= bfd final link relocate");
...
bfd_boolean
_bfd_xgate_elf_merge_private_bfd_data (bfd *ibfd, bfd *obfd)
{
  ...
  elf_elfheader (obfd)->e_flags |= new_flags & EF_XGATE_ABI;
  ...
  if ((new_flags & E_XGATE_I32) != (old_flags & E_XGATE_I32))
    {
    (*_bfd_error_handler)
  ...
  if ((new_flags & E_XGATE_F64) != (old_flags & E_XGATE_F64))
    {
    (*_bfd_error_handler)
  ...
  //  if (!EF_XGATE_CAN_MERGE_MACH (new_flags, old_flags))
  //    {
  //      (*_bfd_error_handler)
  //	(_("%B: linking files compiled for HCS12 with "
  //           "others compiled for HC12"), ibfd);
  // ok = FALSE;
  //    }
  //  new_flags = ((new_flags & ~EF_XGATE_MACH_MASK)
  //               | (EF_XGATE_MERGE_MACH (new_flags, old_flags)));

  elf_elfheader (obfd)->e_flags = new_flags;

  new_flags &= ~(EF_XGATE_ABI | EF_XGATE_MACH_MASK);
  old_flags &= ~(EF_XGATE_ABI | EF_XGATE_MACH_MASK);
    ...
    asection *asect, void *arg)
{
  ...
  struct xgate_scan_param* p = (struct xgate_scan_param*) arg;
