...
mc9xgate_find_match(struct mc9xgate_opcode_handle *opcode_handle, unsigned char numberOfModes, unsigned int sh_format);

void
mc9xgate_get_macro_masks(unsigned int *operMask0, unsigned int *operMask1, char *string);

static char *mc9xgate_parse_exp (char *s, expressionS *op);

//int buildMacro(struct opcode, char lineFeed, char writeBuffer);
...
md_assemble (char *input_line)
{
  ...
  fixup_required = 0;
  //unsigned int macroOperMask0 = 0;
  //unsigned int macroOperMask1 = 0;
  //char macroOp0[10] = {0};
  //char macroOp1[10] = {0};
  //unsigned int macroOpsIndex = 0;

  // struct macroOperands *macroExpansion;
  // char macroString[50];
      ...
      if(0) {
        if (*input_line == 'r' || *input_line == 'R')
          ...
          if(*input_line == '#') { /* go past # character */
            ...
            {
            as_bad(_(": expected register name r0-r7 read %s "), input_line );

            }

        for(i = 0; *p && i < 2; i++){ /* loop though macro operand list */
          p = extract_word(p, op_name, 10);
          if(!(opcode_handle = (struct mc9xgate_opcode_handle *) hash_find (mc9xgate_hash, op_name) )){
            as_bad(_("opcode not found in hash"));
            break;
    ...
    while(*str != ' ' && *str != '\t' ){
      if(*str == '-')
        ...
        if (ISDIGIT (r_name[1]))
          ...
          continue;
          as_bad(_(": not digit expected register name r0-r7 read %s or %c and %c"), r_name,r_name[0], r_name[1] );
          }
      }
      str++;
    }
...
mc9xgate_detect_format(char *line_in){
  ...
  char *r_bug3_string = {"r,"}; /* TODO seems there is a bug with the parsing code*/
  char *r_bug4_string = {",r"}; /* TODO seems there is a bug with the parsing code*/
  char *r_bug5_string = {"r,ip"}; /* TODO seems there is a bug with the parsing code*/
...
//int buildMacro(struct mc9xgate_opcode opcode, char lineFeed, char writeBuffer, struct macroExpansion){
...
//		constraints[i] = *opcode.constraints[i];
//	}

//	printf("\n macro expanded %s", constraints);
//	/* build list of opcodes to call */
/* while  */
//	if(*opcode.constraints[i] == ';')
//		i++;
//	extract_word(opcode.constraints[i], op_name, sizeof(op_name));
//	printf("\n macro expanded opcode %s", op_name);
...
/*
 * get operand values
 * build simulated operand string
 * lookup real opcode
 * call operands to build opcode
 */
//}
static struct mc9xgate_opcode*
mc9xgate_find_match(struct mc9xgate_opcode_handle *opcode_handle, unsigned char numberOfModes, unsigned int sh_format){
 ...
 *operMask0 = (unsigned int) strtol(string, &end, 16);
 *operMask1 = (unsigned int) strtol(end, &end, 16);
